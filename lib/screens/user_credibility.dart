...
class _UserCredibilityScreenState extends State<UserCredibilityScreen> {
      ...
      if (user != null) {
        ...
        if (doc.exists) {
          ...
          setState(() {
            ...
            _hasValidID = factors['hasValidID'] ?? false;
            _hasEmployment = factors['hasEmployment'] ?? false;
            _hasBankAccount = factors['hasBankAccount'] ?? false;
            _hasGoodCreditHistory = factors['hasGoodCreditHistory'] ?? false;
            _hasCollateral = factors['hasCollateral'] ?? false;
            _incomeRange = factors['incomeRange'] ?? 0;
  ...
  Future<void> _updateCredibility() async {
    try {
      ...
      if (user != null) {
        ...
        // DON'T navigate back automatically - let user decide when to leave
      }
    } catch (e) {
  ...
  bool get _hasUnsavedChanges {
    ...
    return !_mapsEqual(originalFactors, currentFactors);
  }
